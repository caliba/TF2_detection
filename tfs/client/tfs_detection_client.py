...
def load_img2tensor(image_path):
    ...
    input_tensor = tf.convert_to_tensor(image,dtype='uint8') # tf.float32
...
def load_imgbatch(img_path,batchsize):
    ...
    return tf.convert_to_tensor(batch,dtype='uint8') # float32
...
def inference(model_name, batch):
    ...
    request.inputs['input_tensor'].CopyFrom(tf.make_tensor_proto(input_tensor))
    send_time = timer()
    ...
    for batch in batch_size:
        ...
        request.inputs['input_tensor'].CopyFrom(tf.make_tensor_proto(batches))
        # inference 
